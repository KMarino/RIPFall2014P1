...
def movePlayer(location, curLocation, boxLocations2):
    x,y = location;
    curX, curY = curLocation;
    if curMap[y][x] == 2:
        return False;
    if (abs(curY-y) <2 and abs(curX-x)<2 and location not in boxLocations2):
        #self.playerLocation = location;
        return True;
    else:
        return False;

def canMoveBox(boxLocation1, boxLocations2, playerLocation1):
    px, py = playerLocation1;
    bx, by = boxLocation1;
    posMoves = []
    if (px - bx == 1 and py-by == 0):
        ...
        posMoves = (0,1);   
    moves = []
    if (bx,by+1) not in boxLocations2 and (bx, by-1) not in boxLocations2:
        if curMap[by+1][bx] == 0 and curMap[by-1][bx] <2:
            moves.append((0, 1));
        if curMap[by+1][bx] <2 and curMap[by-1][bx] == 0:
            moves.append((0, -1));
       
    if (bx+1,by) not in boxLocations2 and (bx-1,by) not in boxLocations2:
